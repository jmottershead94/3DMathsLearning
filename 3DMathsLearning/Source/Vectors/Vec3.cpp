...
Vec3 Vec3::add(const Vec3& v1)
{
	Vec3 sum
	(
		(this->elements_[0] + v1[0]),
		(this->elements_[1] + v1[1]),
		(this->elements_[2] + v1[2])
	);

	return sum;
}

Vec3 Vec3::subtract(const Vec3& v1)
{
	Vec3 sub
	(
		(this->elements_[0] - v1[0]),
...
{
	return std::string("X = " + std::to_string(elements_[0]) + " Y = " + std::to_string(elements_[1]) + " Z = " + std::to_string(elements_[2]));
}
